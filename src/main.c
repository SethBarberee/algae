...
void server_new_keyboard(struct algae_server *server,
		struct wlr_input_device *device) {
	struct wlr_keyboard *wlr_keyboard = wlr_keyboard_from_input_device(device);

	struct algae_keyboard *keyboard = calloc(1, sizeof(*keyboard));
...
static struct algae_view *desktop_view_at(
	 ...
	 * surface in the surface tree of a algae_view. */
	...
	*surface = scene_surface->surface;
	/* Find the node corresponding to the algae_view at the root of this
...
static void reset_cursor_mode(struct algae_server *server) {
...
static void output_frame(struct wl_listener *listener, void *data) {
	...
	struct algae_output *output = wl_container_of(listener, output, frame);
...
static void output_request_state(struct wl_listener *listener, void *data) {
	...
	struct algae_output *output = wl_container_of(listener, output, request_state);
	const struct wlr_output_event_request_state *event = data;
	wlr_output_commit_state(output->wlr_output, event->state);
}

static void output_destroy(struct wl_listener *listener, void *data) {
	struct algae_output *output = wl_container_of(listener, output, destroy);
...
static void server_new_output(struct wl_listener *listener, void *data) {
	...
	struct algae_output *output = calloc(1, sizeof(*output));
...
static void xdg_toplevel_request_maximize(
	 ...
	 * client-side decorations. algae doesn't support maximization, but
...
static void server_new_xdg_surface(struct wl_listener *listener, void *data) {
	...
	struct algae_view *view = calloc(1, sizeof(*view));
	view->server = server;
